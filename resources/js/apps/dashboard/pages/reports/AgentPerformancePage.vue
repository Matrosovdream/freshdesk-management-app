...
async function load() {
    await reports.fetch('agent-performance', { from: range.value[0], to: range.value[1] });
}
async function exportCsv() {
    const url = await reports.export('agent-performance', { from: range.value[0], to: range.value[1] });
...
<template>
    <div class="flex flex-col gap-4">
        <div class="flex items-center justify-between flex-wrap gap-3">
            <h1 class="text-2xl font-semibold">Agent performance</h1>
        ...
        <DataTable :value="reports.agentPerformance?.rows || []" :loading="reports.loading['agent-performance']" stripedRows>

...
Enumeration of every permission slug used by the app. Lives in one PHP class so seeders, policies, middleware, and UI all read from the same source.

Grouping convention: slug = `<area>.<action>`. The area is the migration's `role_rights.group` column.

---

## `app/Support/Rights.php`
...
final class Rights
{
    ...
    // ---- Roles (read-only from Freshdesk side) ----
    ...
    /**
     * Flat catalog keyed by group. Used by seeders and UI.
     */
...
## Role → rights map (used by `RoleRightsSeeder`)

| Role | Rights |
|---|---|
| **superadmin** | Every right in `catalog()` **except** anything starting with `portal.` |
| **manager** | `tickets.view/create/update/delete/restore/bulk_update/merge/forward/assign`, all `conversations.*` except `delete`, `contacts.view/create/update/delete/restore/merge/send_invite`, `companies.view/update`, `agents.view`, `groups.view`, `roles.view`, `time_entries.view/create/update`, `reports.view/export` |
| **customer** | All `portal.*` rights |

Seed code implementing this map lives in [01-data-layer.md § 5](01-data-layer.md).
